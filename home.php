<?php
/**
 * Gold Moment Tattoo Bali — Blog Index (Posts Page)
 *
 * WordPress uses this file when a page is set as the "Posts page"
 * under Settings → Reading. Template hierarchy: home.php > index.php
 */
...
<section class="blog-main-section" aria-label="Blog Posts">
    <div class="container">
        ...
        <!-- No results (shown by JS when filter/search finds nothing) -->
...
.blog-search-box  { position: relative; width: 100%; max-width: 480px; }
.blog-search-icon {
...
/* ── Main Section ──────────────────────────────────────── */
.blog-main-section { padding: 60px 0 100px; }
...
.blog-filter-tab:hover { border-color: rgba(201,168,76,0.5); color: var(--white); }
.blog-filter-tab.active { background: var(--gold); border-color: var(--gold); color: var(--black); }
.blog-tab-count {
...
.blog-filter-tab.active .blog-tab-count { background: rgba(0,0,0,0.25); color: var(--black); }
...
.blog-card.is-filtered-in { animation: cardFadeIn 0.35s ease forwards; }
...
/* Cover link */
.blog-card-cover-link { position: absolute; inset: 0; z-index: 1; display: block; }

/* Thumbnail */
.blog-card-img { position: relative; overflow: hidden; height: 240px; flex-shrink: 0; }
...
.blog-card-cat {
    ...
    padding: 4px 11px; border-radius: 2px; z-index: 2;
}
...
.blog-card-img-overlay i { font-size: 1.6rem; color: var(--gold); filter: drop-shadow(0 0 12px rgba(201,168,76,0.6)); }
...
.blog-card-meta { display: flex; gap: 14px; margin-bottom: 10px; font-size: 11px; color: #666; flex-wrap: wrap; }
.blog-card-meta i { margin-right: 4px; color: var(--gold); }
.blog-card-title {
    font-family: var(--font-heading); font-size: 1.05rem; font-weight: 600;
    letter-spacing: 0.04em; line-height: 1.45; margin-bottom: 10px;
    color: var(--white); transition: color 0.3s;
}
.blog-card:hover .blog-card-title { color: var(--gold); }
.blog-card-excerpt { font-size: 0.85rem; color: #777; line-height: 1.7; flex: 1; margin-bottom: 18px; }
...
/* ── No Results ────────────────────────────────────────── */
.blog-no-results { text-align: center; padding: 60px 20px; }
.blog-no-results i { font-size: 2.5rem; margin-bottom: 20px; display: block; }
.blog-no-results h3 { font-family: var(--font-heading); font-size: 1.4rem; color: var(--white); margin-bottom: 10px; }
.blog-no-results p { color: var(--gray); font-size: 0.9rem; }

/* Results counter */
.blog-results-count { font-size: 11px; color: #555; letter-spacing: 0.1em; text-align: center; margin-top: 32px; }
</style>
...
(function () {
    ...
    function applyFilters() {
        ...
        if (counter)  counter.textContent    = visible > 0 ? 'Showing ' + visible + ' article' + (visible !== 1 ? 's' : '') : '';
    }
